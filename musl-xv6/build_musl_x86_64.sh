...
# ── Add __NR_ aliases to sysroot syscall.h ─────────────────────────────
# musl's Makefile generates SYS_ defines from __NR_ defines but NOT the
# reverse.  GDK/GTK and other Linux-origin code checks __NR_ names
# (e.g. __NR_memfd_create) so we synthesize them from the SYS_ entries.
SYSROOT_SYSCALL_H="${PREFIX}/include/bits/syscall.h"
if [ -f "${SYSROOT_SYSCALL_H}" ]; then
    sed -n 's/^#define SYS_\(.*\)/#define __NR_\1/p' "${SYSROOT_SYSCALL_H}" \
        | while read -r line; do
            name=$(echo "$line" | awk '{print $2}')
            grep -q "^#define ${name} " "${SYSROOT_SYSCALL_H}" || echo "$line" >> "${SYSROOT_SYSCALL_H}"
        done
    echo "Added __NR_ aliases to ${SYSROOT_SYSCALL_H}"
fi
